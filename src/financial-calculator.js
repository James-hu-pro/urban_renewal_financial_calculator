...
class UrbanRenewalCalculator {
  ...
  static validateParams(params) {
    ...
    if (params.exchangeRatio !== undefined) {
      if (params.exchangeRatio < 0) errors.push('exchangeRatio 不能为负数');
    ...
    // ---- 逻辑约束 ----
    // 当 exchangeRatio=0 时，为全部收储模式，不校验调换面积
    const isDirectAcquisition = params.exchangeRatio === 0;
    
    // 地上新建 > 地上产权调换（否则没有超配面积可销售）
    if (!isDirectAcquisition && params.aboveGroundNew !== undefined && params.aboveGroundExchange !== undefined &&
    ...
    // 地下新建 > 地下产权调换
    if (!isDirectAcquisition && params.undergroundNew !== undefined && params.undergroundExchange !== undefined &&
  ...
  calculate() {
    ...
    let belowExchange = p.undergroundExchange;
    
    // 当产权调换比例为0时，强制调换面积为0（直接征收重建模式）
    if (p.exchangeRatio === 0) {
      aboveExchange = 0;
      belowExchange = 0;
    } else if (aboveExchange === null) {
      const aboveRatio = p.aboveGroundNew / totalNewArea;
      aboveExchange = totalExchangeArea * aboveRatio;
    }
    // belowExchange 使用用户传入值（当exchangeRatio=0时已强制为0）
